...
- [Core.lua](Core.lua) — addon entry point. Creates the `BestAround` addon object via `LibStub("AceAddon-3.0"):NewAddon(...)` (with `AceEvent-3.0` and `AceConsole-3.0` mixins). Key lifecycle:
  ...
  - Event handler methods (`BestAround:ACHIEVEMENT_EARNED`, `BestAround:PLAYER_LEVEL_UP`, `BestAround:PLAYER_DEAD`) each check `self.db.profile.<category>.enabled` and then call `BestAround:PlayCategorySound(category)`, which plays `baseSoundPath .. self.db.profile[category].soundFiles` on `soundChannel` via `PlaySoundFile`. Test buttons and `/bar test ...` call `PlayCategorySound` directly, so they bypass the enabled toggle.
...
3. Add a `BestAround:<EVENT_NAME>(event, ...)` handler in Core.lua that checks `self.db.profile.<category>.enabled` and calls `self:PlayCategorySound("<category>")`.
